...
<%!
//-----------------------------------------------
void insert(ConditionalForwarderDao dao){
	ConditionalForwarderData data = new ConditionalForwarderData();
	data.id = paramInt("id");
...
//-----------------------------------------------
void delete(ConditionalForwarderDao dao){
...
ConditionalForwarderDao dao = new ConditionalForwarderDao();
...
<div class="container-fluid primary" style="margin-top:-5px;">
	<ol class="breadcrumb" style="margin-left:10px; margin-right:10px;">
		...
		<li class="breadcrumb-item text-info"><%= translate("CONDITIONAL FORWARDER")%></li>
	</ol>
</div>
<!-- /Breadcrumb -->
...
<div id="listDiv" class="container-fluid" style="display: none;">
	<div class="m-2 expand-lg">
		<table id="list" class="cell-border hover" style="width:100%">
			...
			<tbody>
<%
List<ConditionalForwarderData> dataList = dao.selectList();
for(int i = 0; i < dataList.size(); i++){
	ConditionalForwarderData data = dataList.get(i);
%>
				<tr>
					<td><%= data.domain%></td>
					<td><%= data.ip%></td>
...
function actionDelete(id, domain){
	if(!confirm('<%= translate("Deleting conditional forwarder setup")%> : ' + domain)){
		return;
	}
